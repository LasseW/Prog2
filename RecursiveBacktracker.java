...
public class RecursiveBacktracker implements MazeGenerator {
    ...
    private boolean validNewPosition(char[][] maze, int x, int y, int[] offset) {
        ...
        if (newY < 0 || newY >= maze.length || newX < 0
                || newX >= maze[0].length) {
            return false;
        }
    ...
    private char[][] border(char[][] maze) {
        ...
        for (int i = 0; i < borderedMaze.length; ++i) {
            for (int j = 0; j < borderedMaze[0].length; ++j) {
                if (i == 0 || j == 0 || i == borderedMaze.length - 1
                        || j == borderedMaze[0].length - 1) {
     ...
     */
    private void buildHallway(char[][] maze, int curX, int curY, int[] offset,
            int length) {
        ...
        for (int i = 1; i <= length; ++i) {
            ...
            if (curY < 0 || curY >= maze.length || curX < 0
                    || curX >= maze[0].length) {
                return;
            }
